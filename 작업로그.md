...
#### 7. 모바일 레이아웃 개선
- 멤버 현황 | 오늘의 학습시간 | 실시간 기록 배치 개선
- 모바일: 멤버 현황(전체 너비) + 학습시간/실시간기록(한 줄에 50%씩)
- 데스크톱: 세 섹션 모두 한 줄에 배치
- `grid-cols-2 sm:grid-cols-3` + `col-span-2 sm:col-span-1` 활용
...
- [x] 모바일 레이아웃 개선 (학습시간/실시간기록 한 줄 배치)

---

## 2026년 2월 3일 - 면접 연습 기능 대폭 확장

### 🔧 작업 내용

#### 1. 사례 관리 페이지 UI 통일 (/interview/admin)
- 전반적인 텍스트 크기 축소 (xs, 10px)
- 버튼 디자인 파스텔 톤으로 변경 (indigo-100, blue-100 등)
- 입력 폼 및 모달 컴팩트화
- 다른 페이지와 일관된 UI 적용

#### 2. 면접 답안 DB 영구 저장 기능
- `interview_answers` 테이블 신규 생성
- 답안 자동 저장 (1초 디바운싱)
- 체크포인트 상태도 함께 저장
...
#### 3. 답안 작성 상태 표시
- 질문 카드에 번호 버튼 추가
- 답안 작성된 질문: ✓ 표시 + 에메랄드 색상
- 현재 질문: 인디고 색상
- 미작성 질문: 회색
- 클릭하여 해당 질문으로 이동 가능

#### 4. 내 답안 모아보기 페이지 (/interview/my-answers)
- 작성한 모든 답안을 사례별로 그룹화
- 전공/윤리 필터 기능
- 사례 카드 펼치기/접기
- 답안 삭제 기능
...
#### 5. 연습 카운트 로직 변경
- **기존**: 타이머 30초 이상 + 다음 사례 이동 시 카운트
- **변경**: 답안 저장 시 자동 카운트
- 같은 사례는 하루에 1번만 카운트 (중복 방지)
- `practicedCasesToday` Set으로 추적

#### 6. UI/텍스트 변경
- 제목: "면접 시뮬레이터" → "면접 연습"
- "답안 확인하기" 버튼 파스텔 톤으로 디자인 통일

#### 7. Pretendard 폰트 적용
- CDN 방식으로 Pretendard 웹폰트 적용
- tailwind.config.js에 font-pretendard 추가
- Inter 폰트를 fallback으로 유지
...
├── app/
│   ├── interview/
│   │   ├── page.js              # 면접 연습 (답안 저장, 상태 표시)
│   │   ├── admin/
│   │   │   └── page.js          # 사례 관리 (UI 통일)
│   │   └── my-answers/
│   │       └── page.js          # 내 답안 모아보기 (신규)
│   └── layout.js                # Pretendard 폰트 추가
├── tailwind.config.js           # font-pretendard 설정
└── supabase_interview_answers.sql  # 답안 테이블 SQL
```

#### 8. 사례 헤더 정보 간소화
- 답안 확인 후에도 불필요한 정보 숨김 처리
- **표시 유지**: 고유번호 뱃지, 출제연도 뱃지
- **제거**: 카테고리 뱃지, 🔮 예상 뱃지, 사례 제목/진단명 섹션

#### 9. D-day 한국 시간대(KST) 수정
- 기존: UTC 기준으로 계산되어 시간대 오류 발생
- 변경: 한국 표준시(KST, UTC+9) 기준으로 계산
- 브라우저 시간대와 관계없이 항상 한국 시간 기준 D-day 표시
...
- [x] 답안 작성 상태 표시 (✓ 버튼)
- [x] 내 답안 모아보기 페이지
- [x] 연습 카운트 로직 변경 (답안 저장 기준)
- [x] "면접 시뮬레이터" → "면접 연습" 변경
- [x] 답안 확인하기 버튼 디자인 통일
- [x] Pretendard 폰트 적용
- [x] 사례 헤더 정보 간소화 (고유번호+출제연도만 표시)
- [x] D-day 한국 시간대(KST) 수정
